...
DEFAULT_CONFIG = {
    ...
    # 设备过滤器 - 默认为空，在 Grafana 中自行配置
    'device_filter_increase': '',
}
...
{metric_name}{{instance="{instance}", reset_day="{reset_day}"}} {value}
"""
    else:
        payload = f"""\
# HELP {metric_name} {help_text}
# TYPE {metric_name} gauge
{metric_name}{{instance="{instance}"}} {value}
"""
...
def process_instances(config):
    ...
    for item in instances:
        ...
        logger.debug(f"--- 处理实例: {instance_id}, 重置日: {reset_day} ---")
